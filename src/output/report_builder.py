...
class ReportBuilder:
    ...
    def _merge_similar_insights(self, insights: List[str]) -> List[str]:
        ...
        for i, insight1 in enumerate(insights):
            ...
            for j, insight2 in enumerate(insights[i + 1 :], i + 1):
                if (
                    j not in used
                    and self._calculate_similarity(insight1, insight2)
            ...
            if section_pattern in content:
                ...
                if section_text:
                    ...
                    insights.extend(
                        [
                            insight.strip()
                            for insight in formatted_text.split("\n\n")
                            if insight.strip()
                        ]
                    )
    ...
    def _format_content(self, text: str, source: str) -> str:
        ...
        for line in text.split("\n"):
            ...
            # Sub-points should be indented
                ...
                or line.startswith("- Effects:")
                or line.startswith("- Limitations:")
                or line.startswith("- Timeframe:")
            ):
                current_item.append("    " + line)
    ...
    def _build_summary_section(
        self, section: str, results: List[Dict[str, str]]
    ) -> List[str]:
